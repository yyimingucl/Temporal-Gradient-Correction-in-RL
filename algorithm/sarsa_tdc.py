# -*- coding: utf-8 -*-
"""
Created on Thu Apr 23 04:45:02 2020
...
def sarsa_tdc_function_approx(
    ...
    for _ in range(num_episodes):
        ...
        while not env.is_terminal() and steps < max_steps:
            ...
            """
            # update the weights
            qweights,lms = sarsa_tdc_update(qweights, s_rep, a, r, next_s_rep, next_a, 
                                        gamma, alpha,lms,beta)
...
def sarsa_tdc_update(qweights, s_rep, a, r, next_s_rep, next_a, 
                 gamma, alpha, lms, beta):
    """
    parameters
    ----------
    qweights - a list of weight vectors, one per action
        qweights[i] is the weights for the ith action
    a,next_a - are the index of the action so for us it is 0, 1, 2 or 3
    s_rep,next_s_rep - are the 1d numpy arraies of the state feature
    lms - is the vector used to minimise the expected bellman error(least mean square)
    beta - learning rate for lms
    alpha - learning rate for VFA
    """
